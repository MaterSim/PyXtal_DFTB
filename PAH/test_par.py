...
def get_codes(codes, disps=['TS', 'D3']):
    codes1 = []
    for code in codes:
        converged = True
        for disp in disps:
            tag = "{:s}-{:s}".format(code, disp)
            F_status = code + '/Status-' + tag + '.txt' 
...
if __name__ == "__main__":
    ...
    if True: #local:
        ...
        cmd = '/home/qzhu/anaconda3/envs/htocsp/bin/dftb+ > PREFIX.out'
    ...
    nproc, total_cpu, step1, step2, vmax = 20, 20, 15000, 300, 2000
    ...
    vols = np.array(vols)
    ids = np.argsort(vols)
    codes = [codes[i] for i in ids]
    codes1 = get_codes(codes)
    print("codes to complete", len(codes), len(codes1))
    print(codes1)
    ...
    for cycle in range(N_cycle):
        ...
        for i in range(N1, N2):
            p = mp.Process(target=run_calc,
                           args = (db, codes1[i], skf_dir, ftol, step1, step2))
            p.start()
            processes.append(p)
